...
// when program starup, do some check and initial works
void checkotherwm(void) {
    // check wheather other window manager is running,
    // if true will call the callback function
...
void runautostart(void) {
    ...
    if (!(stat(pathpfx, &sb) == 0 && S_ISDIR(sb.st_mode))) {
        ...
        // In computer, program will execute ~/.dwm/autostart.sh
...
void setup(void) {
    ...
    // find the target window or get the default
    xinitvisual();

    // drawable window abstraction
...
void xinitvisual() {
    ...
    // if can't find the target visual window, get the default
    if (!visual) {
...
int main(int argc, char *argv[]) {
    ...
    scan(); // update windows setting

    runautostart(); // run startup script
